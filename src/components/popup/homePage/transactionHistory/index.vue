...
let queueTransactions: any = ref([]); //进行中的交易
let web3: any = ref(null);

let currentWallt: any = ref(null); //当前钱包信息
let rpc_url: any = ref(null); //当前网络
let rawData: any = ref(null);
let transactionPage = ref('list'); //当前展示页面
let transactionTabs = ref('dispose'); //完成的还是处理中
// 详情页
...
// 时间转换
// const getTime = () => {
//   web3.value.eth.getBlock(
//     transactionDetail.value.blockNumber,
//     (error, block) => {
//       if (!error) {
