...
onMounted(async () => {
	keyStore.value = await indexDbData.getData('keyStore');
})
// 确认
const privatePrivateConfirm = async () => {
    ...
    // 私钥加密
	let keyList = keyStore.value.secret;
	let yon:any = false;//是否存在当前的私钥
	for (let key in keyList) {
        console.log(keyList[key])
		let encryption = await Decrypt(keyList[key], passKey.value);
		console.log(encryption, 'encryption');
		if(encryption == privatePhrase.value){
			yon = true;
		}
    }
	console.log(yon)
	if(yon){
		bus.emit('promptModalErr', '该账户已存在');
		return;
	}
    let keyName = uuidv4();
...
const isValidPrivateKey = async (keyName: any) => {
    try {
        const web3 = new Web3();
        const account: any = web3.eth.accounts.privateKeyToAccount(privatePhrase.value);
...
const evmNetwork = (walltInfo: any) => {
    indexDbData.getData('EVM').then((res: any) => {
        ...
        Object.keys(data.content).forEach((item) => {
            ...
            account['keyStoreType'] = 'privateKey';
            account['userName'] =
                'Wallt' + (!data['NoIndex'] ? '01' : data['NoIndex'] > 10 ? data['NoIndex'] : ' 0' + data['NoIndex']);
