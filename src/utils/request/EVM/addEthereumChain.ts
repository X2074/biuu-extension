...
// 向钱包添加以太坊链 (EIP-3085)
export default async function addEthereumChain(request: any): Promise<any> {
    ...
    try {
        ...
        if (!chainId || typeof chainId !== 'string' || !/^0x[1-9a-fA-F][0-9a-fA-F]*$/.test(chainId)) {
            const err:any = new Error(`Expected 0x-prefixed, unpadded hex string "chainId". Received: ${chainId}`);
            err.code = -32602;
            throw err;
        }
        let chainIdNum:any;
        ...
        try {
            ...
            if (chainIdNum > Number.MAX_SAFE_INTEGER) {
                const err:any = new Error(`Invalid chain ID "${chainId}": numerical value greater than max safe value. Received: ${chainId}`);
                err.code = -32602;
                throw err;
            }
        } catch (e:any) {
            const err:any = new Error(`Invalid chain ID: ${e.message}`);
            err.code = -32602;
            throw err;
        }
        ...
        if (!Array.isArray(rpcUrls) || rpcUrls.length === 0 || 
            ...
            const err:any = new Error('Expected an array with at least one valid HTTPS URL "rpcUrls"');
            err.code = -32602;
            throw err;
        }
        ...
        if (!nativeCurrency || typeof nativeCurrency !== 'object') {
            const err:any = new Error('Missing or invalid nativeCurrency');
            err.code = -32602;
            throw err;
        }
        ...
        if (!nativeCurrency.symbol || typeof nativeCurrency.symbol !== 'string' || 
            ...
            const err:any = new Error(`Expected 2-6 character string 'nativeCurrency.symbol'. Received: ${nativeCurrency.symbol}`);
            err.code = -32602;
            throw err;
        }
        ...
        if (typeof nativeCurrency.decimals !== 'number' || 
            ...
            const err:any = new Error('Invalid nativeCurrency.decimals. Must be between 0 and 36');
            err.code = -32602;
            throw err;
        }
console.log(chainIdNum,"chainIdNum");
        ...
        if (blockExplorerUrls !== undefined) {
            if (!Array.isArray(blockExplorerUrls) || 
                ...
                const err:any = new Error('Expected null or an array with at least one valid HTTPS URL "blockExplorerUrls"');
                err.code = -32602;
                throw err;
            }
        }
        ...
        // 其他错误
        const err:any = new Error(error.message || 'Failed to add chain');
