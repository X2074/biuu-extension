...
export async function requestPermissions(request: any) {
    try {
        ...
        const popup = await chrome.windows.create({
            url: chrome.runtime.getURL('popup/index.html#/connect') + `?origin=${encodeURIComponent(currentOrigin)}`,
            type: 'popup',
            width: 400,
            height: 600
        });
                                ...
                                resolve([
                                    {
                                        ...
                                        caveats: [
                                            ...
                                            {
                                                type: 'expiration',
                                                value: authorizedSites[currentOrigin].expiresAt
                                            }
                                        ]
                                    }
                                ]);
...
export async function requestAccountsWallt(params: any) {
    ...
    // 检查是否已有授权
    const authorizedSites = (await indexDbData.getData('authorized_sites')) || {};
    // 在 requestAccountsWallt 函数中
    if (authorizedSites && authorizedSites[currentUrl]) {
        // 检查是否已过期
        const siteInfo = authorizedSites[currentUrl];
        if (siteInfo.expiresAt && Date.now() > siteInfo.expiresAt) {
            // 已过期，删除授权信息
            delete authorizedSites[currentUrl];
