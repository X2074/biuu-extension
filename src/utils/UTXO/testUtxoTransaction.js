...
async function testTxSign() {
  ...
  // 获取发送方地址的余额
  const balance1 = (await getUTXOBalance(address)).result / 100000000
  console.log(balance1)
  // 获取接收方地址余额
  const balance2 = (await getUTXOBalance(targetAddress)).result / 100000000
  ...
  [
    {
      type: 'normal',
      amount: 50000000000,
      txid: 'e569967c9dfc34dbe34e5d73d4e75e94d032e26a7221eccf322fb82e41cd7f27',
      idx: 0,
      status: 'valid'
    }
  ]
  ...
  txb.addOutput(address, 49900000000);
  console.log("txb:", txb)
  // 使用前面通过私钥生成的密钥对签署交易
  utxos.map((v, i) => {
    txb.sign(i, keyPair);
  ...
  // 发送交易
  const response = await sendTraction('testnet', [newTransaction, false])
